...
void main() {
  group('DaayakattaiGame Rules Tests', () {
    ...
    test('Pairs (Jodu) Blocking - Single piece cannot land on safe cell double occupied', () {
      ...
      // Statically place 2 pieces of Player 0 on unsafe cell index 5
      game.debugSetupPiece(0, 0, PieceState.outer, outerSteps: 5);
      game.debugSetupPiece(0, 1, PieceState.outer, outerSteps: 5);

      // Verify they form a pair on outer cell 5
      expect(p0.pieces[0].outerIndex, equals(5));
      expect(p0.pieces[1].outerIndex, equals(5));

      // Attempt to move Player 1's piece onto cell 5
      game.debugSetupPiece(1, 0, PieceState.outer, outerSteps: 0);
      
      // Let's manually check if player 1 landing on cell 5 is valid
      // Since cell 5 is unsafe and occupied by Player 0's pair, it should block Player 1.
      // In twoPlayer/fourPlayer, start indices are:
      // P1 starts at Board.startOuterIndices[1] = 9.
      // If P1 moves 2 steps backward or forward, they shouldn't land on cell 5 if blocked.
